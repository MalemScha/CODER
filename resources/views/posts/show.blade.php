...
@section('content')
    <div class="container">
        <div class="row">
            ...
            <post-view :initial-count="{{ $post->comments_count }}" inline-template>
                <div class="col-md-9 col-md-push-3 primary">
                    <img class=" mr-1" style="width: 100px; height: 100px; float:left; border-radius: 50%; top: 7px; left: -30px;" src="{{ Storage::url( $post->creator->avatar ) }}" alt=""/>
                    <h3 class="flex">{{$post->title}}</h3>
                    <div class="level">
                        Posted by :<a href="{{url('/@' . $post->creator->name)}}" class="flex">{{$post->creator->name}}</a>
                        <span v-text="repliesCount" ></span><i class="fa fa-commenting mr-1" aria-hidden="true"></i> <div class="btn btn-sm btn-primary mr-1"> {{ $post->channel->name}}</div>
                    </div>
                    <div class="level">
                        <span class="mr-1">{{$post->created_at->diffForHumans()}}</span>

                        @can ('update', $post)
                            <a class="mr-1 btn-xs btn btn-success" href="{{ url($post->path() . '/edit')}}">edit</a>
            ...
            <div class="col-md-3 col-md-pull-9">
                ...
                <aside class="forum-sidebar menu column is-3">
                    @if (auth()->check())
                        <a href="{{url('/forum/create')}}"><button class="btnn mb-2">START A DISCUSSION</button></a>
                    @else
                    ...
                    <ul class="menu-list">
                        ...
                        <li>
                            <a href="/forum?by=Malemnganba%20Soubam"
                               class=" has-icon"
                            >
