...
@section('content')
    <div class="container">
        <div class="row">
    <challenge-view :initial-count="{{ $challenge->replies_count }}" inline-template>
                ...
                <div class="col-md-9 col-md-push-3 primary">
                    <img class=" mr-1" style="width: 100px; height: 100px; float:left; border-radius: 50%; top: 7px; left: -30px;" src="{{ Storage::url( $challenge->maker->avatar) }}" alt=""/>
                        <h4 class="flex">{{$challenge->title}}</h4>
                        <div class="level">
                            Posted by :<a href="{{  url('/@' . $challenge->maker->name) }}" class="flex">{{$challenge->maker->name}}</a>
                        </div>
                        <div class="level">
                            {{$challenge->created_at->diffForHumans()}}
                            @can ('update', $challenge)
                                <form action="{{ $challenge->path() }}" method="post">
                                    {{ csrf_field() }}
                                    {{ method_field('DELETE') }}
                                    <button type="submit" class="btn btn-xs btn-danger" style="margin-left: 1em">Delete</button>
                                </form>
                            @endcan
                        </div>
                    ...
                    <div class="panel-body">
                        ...
                        <div class="less-margin-2 input-output-container">
                            <div class="input-output right-border">
                                ...
                                <div class="dark"><pre>{{$challenge->sample_input}}</pre></div>
                            </div>
                            <div class="input-output">
                                ...
                                <div class="dark"><pre>{{$challenge->sample_output}}</pre></div>
                            </div>
                            <div class="clear"></div>
                        </div>
                    </div>
                    ...
                    <div class="tab_container">
                        ...
                        <label for="tab2"><i class="fa fa-pencil-square-o"></i><span>DISCUSSION (<span v-text="repliesCount"></span></span>)</label>

                        {{--<input id="tab3" type="radio" name="tabs">--}}
                        {{--<label for="tab3"><i class="fa fa-bar-chart-o"></i><span>RANKINGS</span></label>--}}
                        <section id="content1" class="tab-content">
                            <h3>Supported Languages</h3>
                            <p>C, C++ Java, Java 8, JavaScript, Objective-C, PHP, Python, Python 3, Racket, Ruby </p>
                            @if(Auth()->check())

                            <form action="{{$challenge->path()}}/files" method="post" enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <label class="labb" for="file-input">Submit or Change your code</label>
                                <input type="file" name="filenam" id="file-input" required>
                                @if ($errors->has('filenam')) <p class="alert alert-danger">{{ $errors->first('filenam') }}</p> @endif
                                <button class="btnnn" type="submit">SUBMIT</button>
                            </form>
                            @endif
                            <h2>Submissions</h2>

                            @forelse($challenge->uploads as $upload)
                                @include("challenges.upload")


                            @empty
                                <p>No attempt so far</p>
                            @endforelse

                        </section>

                        <section id="content2" class="tab-content">
                            <comments @removed=" repliesCount-- " @added=" repliesCount++ " ></comments>
                        </section>
            ...
            <div class="col-md-3 col-md-pull-9">
                ...
                <aside class="forum-sidebar menu column is-3">
                    ...
                    <ul class="menu-list">
                        ...
                        <ul class="menu-list">
                            <li>
                                <a href="/challenge/filter/easy" class=" has-icon">
                    ...
                    <ul class="menu-list">
                        ...
                        <li>
                            <a href="/challenges/filter/php" class=" has-icon">
                                <span class="is-circle icon" style="color: #8cd362"></span>
                                PHP
                            </a>
                        </li>
                        <li>
                        <a href="/challenges/filter/cpp" class=" has-icon">
